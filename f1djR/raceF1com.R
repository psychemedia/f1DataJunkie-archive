...
#mktitle=function(subtitle,event='Singapore',year='2012') return(paste('F1 ',year,event,'-',subtitle))
mktitle=function(subtitle,year='2012') return(paste('F1 ',year,subtitle))


#----
event="Japan"
raceflaps=floader("raceFastlaps",event)
xraceflaps=configData(raceflaps,event)

racepits=floader("racePits")
xracePits=configData(racepits,event)

raceResults=floader("raceResults")
xraceResults=configData(raceResults,event)
...
g=g+ggtitle(mktitle(paste(event,"Race - Normalised Fastest Laptimes")))
...
g=g+ggtitle(mktitle(paste(event,"Race - Fastest Lap Delta to Fastest Overall")))
...
g=g+ggtitle(mktitle(paste(event,"Race - Pit Stops by Lap")))+xlab("Lap")
g=g+ylab("Pit time (s)")
...
g=xRot(g)
g=g+ggtitle(mktitle(paste(event,"Race - Cumulative Pit Stop Times")))
...
xxracePits$team=orderTeams(xxracePits$team)

g=ggplot(xxracePits)+geom_text(aes(x=lap,y=pitTime,label=TLID,col=factor(stops)),size=4,angle=45)
g=g+ggtitle(mktitle(paste(event,"Race - Pit Stops by Lap")))+xlab("Lap")
g=g+ylab("Pit time (s)")
g=g+theme(legend.position="none")
print(g)
...
g=ggplot(xxracePits)+geom_boxplot(aes(x=team,y=pitTime))
