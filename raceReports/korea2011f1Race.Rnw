...
\title{F1 2011 Korea (Race)\\A Statistical Graphics Review}

\author{
Tony Hirst\thanks{@psychemedia, http://f1datajunkie.blogspot.com}\\F1DataJunkie Blog 
}
...
proximity$posdelta=c(0,diff(proximity$pos))
...
p=ggplot(racestats,aes_string(x=timeData,y=speedData,label=driverName))+ geom_text(size=4,angle=45)+scale_x_continuous(limits=speedRange)+opts(legend.position = "none") + scale_x_continuous('Time (s)')+ scale_y_continuous('Speed (km/h)')
print(p)
}
...
p=ggplot(posData) + geom_line(aes(x=lap,y=pos,group=driverName,col=car)) + geom_point(aes(x=lap,y=pos*stop,col=car)) + scale_y_discrete( "Driver",limits=1:24,breaks=1:24) + scale_x_continuous('Lap') + opts(legend.position="none")
print(p)
}
...
\newpage
\subsection{Stint Summary}
The \emph{stint summary} chart summarises stints, defined for the purposes of this graphic as periods between pit entries/exits. As well as pit stops, drive through penalties and stop-and-go penalties also result
...
\begin{center}
<<fig = T, echo = F>>=
p=ggplot(times)+geom_point(aes(y=driverName, x=lap,col=stint,size=3)) + scale_x_continuous('Lap') + scale_y_discrete('Driver') + opts(legend.position="none")

print(p)
@
\caption{Stint Summary; colours denote stint number. \emph{(Note that drive-through/DT and stop-and-go penalities result in an increase in the stint count.)}}
\end{center}
\end{figure}
\noindent\textbf{Things to look for} Does the occurrence and number of stints suggest different pit stop strategies were in place?

\noindent\textbf{Notes} I have to admit, I don't find this chart very informative. Is there a better way of presenting the information that allows us to more easily compare stint lengths? Maybe colouring by tyre would make this chart more useful, perhaps using an underlaid cross to highlight PIT laps on occasions where drivers move onto the same set of tyres they were using in the previous stint?

\newpage
\subsection{(Sector) Times and Speeds}
How do the drivers compare in terms of speed and laptime over the race as a whole, as well as in sector?
...
\newpage
\section{Feedback}
This report format is something of an experiment, so all comments appreciated, particularly on:
...
\item \emph{scripts}: the latest version of the "source code" for this document is available from: \emph{}
