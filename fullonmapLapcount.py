				...
				if 'telemetry' in data['drivers'][drivername]:
					...
					if started==True:
						if racing==False:
							...
							oldDist = d['sLap']
							if f2i<=maxlaps:
								csv_file.writerow([ i,d['timestamp'],d['NGPSLatitude'],d['NGPSLongitude'], d['NGear'],d['nEngine'],d['rThrottlePedal'],d['pBrakeF'],d['gLat'],d['gLong'],d['sLap'],d['vCar'],f2i,str(d['NGPSLatitude'])+','+str(d['NGPSLongitude']) ])
