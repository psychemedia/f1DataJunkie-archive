...
\title{F1 2011 India (Race)\thanks{Source data used to generate this report was obtained from the Timing Information press releases published via the F1/FIA Media Centre }\\A Statistical Graphics Review}
...
speedTime(subset(racestats,inter1>0),'sector1','inter1',c(min(racestats$inter1)-2,max(racestats$inter1)+2))
@
...
speedTime(subset(racestats,inter2>0),'sector2','inter2',c(min(racestats$inter2)-2,max(racestats$inter2)+2))
@
...
speedTime(subset(racestats,finish>0),'sector3','finish',c(min(racestats$finish)-2,max(racestats$finish)+2))
@
...
speedTime(subset(racestats,trap>0),'fastestlap','trap',c(min(racestats$trap)-2,max(racestats$trap)+2))
@
...
p=qplot(x=driverName,y=x, data=subset(minqs,x>0),pch=Stint)+opts(axis.text.x=theme_text(angle=90, hjust=0))+xlab(NULL)+ylab('Laptime (s)')
print(p)
@
...
p=qplot(x=driverName,y=x, data=subset(minqs,x>0),pch=Stint)+opts(axis.text.x=theme_text(angle=90, hjust=0))+xlab(NULL)+ylab('Fuel Corrected Laptime (s)')
print(p)
@
...
The \textbf{HRT} drivers are \emph{NARAIN KARTHIKEYAN (22)} and \emph{Daniel Ricciardo (RIC, 23)}.
